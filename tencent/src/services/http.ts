...
export class Http {
  ...
  public Get(apiUrl: string, query?: any) {
    query = query ? query : {};
    return axios.get(this.BASEURL + apiUrl, {
      params: query,
  ...
  public Post(apiUrl: string, query?: any, headerType?: number) {
    ...
    return axios.post(this.BASEURL + apiUrl, qs.stringify(query), {
      headers,
      timeout: config.timeout
    });
  }
      ...
      response => {
        ...
        switch (status) {
          ...
          case 400:
            return {
              status: false,
              data: response.data ? response.data : "请求错误"
            };
          case 401:
            return {
              status: false,
              data: response.data ? response.data : "请求要求用户的身份认证"
            };
          case 403:
            // router.push({ path: "/exception/403" });
            return {
              status: false,
              data: response.data ? response.data : "网络资源缺失"
            };
          case 404:
            // router.push({ path: "/exception/404" });
            return {
              status: false,
              data: response.data ? response.data : "不存在的资源"
            };
          case 413:
            return {
              status: false,
              data: response.data ? response.data : "上传的资源体积过大"
            };
          case 500:
